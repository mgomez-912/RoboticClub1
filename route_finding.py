...
def generate_random_objects(num_blue=4, num_red=4):
    """
    Generates random positions for objects, ensuring they are within the
    valid grid rows (1, 2, 3) and are unique.
    """
    possible_object_positions = []
    # Objects will be in Rows 1, 2, 3 (0-indexed 1, 2, 3)
    for r in range(1, GRID_ROWS):
        for c in range(GRID_COLS):
            possible_object_positions.append((r, c))

    random.shuffle(possible_object_positions)

    blue_objects = []
    red_objects = []
    chosen_positions = set()

    # Generate unique positions for blue objects
    for _ in range(num_blue):
        if not possible_object_positions:
            raise ValueError("Not enough unique positions for blue objects.")
        pos = possible_object_positions.pop()
        blue_objects.append(pos)
        chosen_positions.add(pos) # Not strictly necessary if popping, but good for clarity

    # Generate unique positions for red objects, avoiding existing blue object positions
    for _ in range(num_red):
        if not possible_object_positions:
            raise ValueError("Not enough unique positions for red objects.")
        pos = possible_object_positions.pop()
        red_objects.append(pos)
        chosen_positions.add(pos) # Not strictly necessary if popping

    return blue_objects, red_objects
...
def solve_robot_challenge_with_red_zones():
    blue_objects, red_objects = generate_random_objects()
    print(f"Generated Blue Objects: {blue_objects}")
    print(f"Generated Red Objects: {red_objects}")
...
if __name__ == "__main__":
    solve_robot_challenge_with_red_zones()
